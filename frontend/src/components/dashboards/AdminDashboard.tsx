    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600">
            State-wide overview of revenue collections and assessments.
          </p>
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-4">
        ...
        <Link href="/admin/institutions" className="rounded-lg bg-blue-50 p-3 hover:bg-blue-100 transition-colors">
          ...
          <div className="text-[11px] text-blue-600">Manage schools</div>
        </Link>
      ...
      {!loading && !error && (
        <div className="space-y-4">
          ...
          {data?.statusCounts?.length ? (
            <div className="rounded-lg bg-white p-4 shadow-sm">
              <StatusBarChart title="Assessments by Status" data={data.statusCounts} />
            </div>
          ) : null}
          ...
          {topLgas.length ? (
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <div className="rounded-lg bg-white p-4 shadow-sm">
                ...
                <div className="mt-2 text-2xl font-semibold text-green-700">{lgaCount}</div>
                <p className="mt-1 text-xs text-gray-600">
                  LGAs with recorded activity.
                </p>
              </div>
            </div>
          ) : null}
